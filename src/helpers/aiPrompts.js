...
function buildPrompts(text) {
  const out = {};
  for (const key of Object.keys(BUILTIN_PROMPT_TEMPLATES)) {
    out[key] = BUILTIN_PROMPT_TEMPLATES[key].replace(/\$\{text\}/g, text);
  }
  return out;
}

const BUILTIN_STYLE_PACK = {
  id: 'builtin-default',
  name: '預設風格',
  isBuiltin: true,
  prompts: BUILTIN_PROMPT_TEMPLATES
};

module.exports = { buildPrompts, BUILTIN_STYLE_PACK };
