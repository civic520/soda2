...
function extractStyleMetadata(content) {
...
function extractDictMetadata(content) {
  const { metadata, body } = parseFrontMatter(content);
  if (metadata && metadata.name) {
    return {
      metadata: {
...
function buildPrompt(settings, customWords = []) {
  const parts = [];
  const activeMainPrompt = settings.activeMainPrompt || "default";
  
  // 1. 主 Prompt
  let mainPromptContent = DEFAULT_MAIN_PROMPT;
  if (activeMainPrompt !== "default" && settings.customMainPrompts?.[activeMainPrompt]) {
    mainPromptContent = settings.customMainPrompts[activeMainPrompt].content;
  }
  parts.push(mainPromptContent);

  // 2. 修飾模式
  const activeMode = settings.activeMode || "general";
  let modeData = DEFAULT_MODES[activeMode];
  if (!modeData && settings.customModes?.[activeMode]) {
    modeData = settings.customModes[activeMode];
  }
  if (!modeData) {
    modeData = DEFAULT_MODES.general;
  }
  const style = extractStyleMetadata(modeData.content);
  parts.push(`---
...
- ${customWords.join(", ")}`);
  ...
  const dictParts = [];
  for (const dictKey of activeDicts) {
    let dictData = DEFAULT_DICTIONARIES[dictKey];
    if (!dictData && settings.customDictionaries?.[dictKey]) {
      dictData = settings.customDictionaries[dictKey];
    }
    if (dictData) {
      const parsed = extractDictMetadata(dictData.content);
      let body = parsed.body;
      // 合併內建詞庫的自訂補充內容
      const extra = settings.customDictionaries?.[dictKey]?.extraContent;
      if (extra && extra.trim()) {
        body = body + "\n\n" + extra.trim();
      }
      dictParts.push(`- **${parsed.metadata.name}**：\n${body}`);
    }
  }
